collect_mec <- function(config) {
  # gov.br MEC uses Plone tileItem structure — identical to IFF
  base <- "https://www.gov.br/mec/pt-br/assuntos/noticias"
  per_page <- 30L
  rows <- list()
  raw_count <- 0L
  ...
  for (page in seq_len(3L)) {
    ...
    page_rows <- tryCatch({
      ...
      log_warn("MEC page failed: {url} - {conditionMessage(e)}")
      tibble::tibble()
    })
  ...
  finish_source_result("MEC", all_rows, raw_count = raw_count, config = config)
}
