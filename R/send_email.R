send_clipping <- function(html, config) {
  ...
  access_token <- tryCatch(
    authenticate_gmail_api(config),
    error = function(e) {
      log_warn("Gmail API authentication failed: {conditionMessage(e)}")
      NULL
    }
  )
  ...
  gmailr_ok <- tryCatch(
    ...
    error = function(e) {
      log_warn("gmailr authentication failed: {conditionMessage(e)}")
      FALSE
    }
  )
  ...
  if (!gmailr_ok) {
    return(list(
      any_success = FALSE,
      dry_run = FALSE,
      per_recipient = tibble::tibble(),
      error = "Gmail authentication failed; no email was sent. Check GMAILR_KEY, GMAILR_TOKEN_ENC_B64 and GMAIL_OAUTH_B64 secrets."
    ))
  }
...
load_gmail_token <- function(config) {
  ...
  if (file.exists(config$gmail_token_path)) {
    ...
    token <- tryCatch(
      readRDS(config$gmail_token_path),
      error = function(e) stop(
        "Gmail token file is corrupted: ", config$gmail_token_path,
        " (", conditionMessage(e), "). Regenerate it with scripts/setup_gmail_automated.R.",
        call. = FALSE
      )
    )
  ...
  if (nzchar(config$gmail_token_enc_b64)) {
    ...
    raw_payload <- tryCatch(
      ...
      error = function(e) stop("Failed to base64-decode GMAILR_TOKEN_ENC_B64: ", conditionMessage(e), call. = FALSE)
    )
    ...
    payload <- tryCatch(
      readRDS(con),
      error = function(e) stop(
        "GMAILR_TOKEN_ENC_B64 does not contain a valid encrypted Gmail token. ",
        "Set this secret to the exact content of secrets/token_base64.txt generated by scripts/setup_gmail_automated.R. ",
        "(readRDS failed: ", conditionMessage(e), ")",
        call. = FALSE
      )
    )
    close(con)
  ...
  if (file.exists(config$gmail_encrypted_token_path)) {
    ...
    payload <- tryCatch(
      readRDS(config$gmail_encrypted_token_path),
      error = function(e) stop(
        "Encrypted Gmail token file is corrupted: ", config$gmail_encrypted_token_path,
        " (", conditionMessage(e), "). Regenerate it with scripts/setup_gmail_automated.R.",
        call. = FALSE
      )
    )
...
refresh_access_token <- function(client_info, refresh_token) {
  tryCatch({
    ...
    # Try to surface the OAuth error body (e.g. invalid_grant) for diagnosis.
    body_txt <- ""
    tryCatch({
      if (exists("resp", inherits = FALSE)) {
        body_txt <- httr2::resp_body_string(resp)
      }
    }, error = function(e2) NULL)
    if (nzchar(body_txt)) {
      err_msg <- tryCatch({
        err <- jsonlite::fromJSON(body_txt, simplifyVector = FALSE)
        paste0(err$error, " - ", err$error_description %||% "")
      }, error = function(e2) substr(body_txt, 1, 300))
      log_warn("Token refresh failed: {conditionMessage(e)} (OAuth: {err_msg})")
    } else {
...
decrypt_gmail_payload <- function(payload, key_text) {
  if (is.null(payload$data) || is.null(payload$iv)) {
    stop(
      "Invalid encrypted Gmail token payload: missing 'data' or 'iv' fields. ",
      "Regenerate the token with scripts/setup_gmail_automated.R.",
      call. = FALSE
    )
  }
  ...
  decrypted <- tryCatch(
    openssl::aes_cbc_decrypt(payload$data, key = key, iv = payload$iv),
    error = function(e) stop("Failed to decrypt Gmail token: ", conditionMessage(e), call. = FALSE)
  )

  token <- tryCatch(
    unserialize(decrypted),
    error = function(e) stop(
      "Failed to deserialize the Gmail token. This usually means GMAILR_KEY does not match the key used to encrypt GMAILR_TOKEN_ENC_B64. ",
      "Regenerate the token (scripts/setup_gmail_automated.R) and update BOTH GitHub secrets (GMAILR_KEY and GMAILR_TOKEN_ENC_B64). ",
